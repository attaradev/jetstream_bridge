...
module JetstreamBridge
  ...
  class Publisher
    ...
    # @param resource_type [String] Type of resource (e.g., "user")
    # @param event_type [String] Type of event (e.g., "created")
    ...
    def publish_with_retries(subject, data, header, retries = DEFAULT_RETRIES)
      attempts = 0
      begin
        @jts.publish(subject, data, header: header)
        ...
        Logging.warn("Publish retry #{attempts} after #{e.class}: #{e.message}", tag: 'JetstreamBridge::Publisher')
        sleep delay
        retry
      end
    end
