...
RSpec.describe JetstreamBridge::Publisher do
  ...
  subject(:publisher) { described_class.new(connection: jts, config: config) }
  ...
  describe '#publish with hash envelope' do
    ...
    it 'requires payload in hash' do
      ...
      end.to raise_error(ArgumentError, /missing keyword.*payload/)
    end
  end
  ...
  describe '#publish parameter validation' do
    ...
    it 'raises error when destination_app is not configured' do
      ...
      bad_publisher = described_class.new(connection: jts, config: bad_config)
      expect do
        bad_publisher.publish(resource_type: 'user', event_type: 'created', payload: payload)
  ...
  describe '#publish with custom subject' do
    ...
    it 'uses subject from options' do
      ...
      result = publisher.publish(event_type: 'created', payload: payload, subject: 'options.subject')
      expect(result).to be_a(JetstreamBridge::Models::PublishResult)
      expect(result.success?).to be(true)
    end
  end
  ...
  describe '#publish with outbox enabled' do
    ...
    let(:outbox_publisher) { described_class.new(connection: jts, config: outbox_config) }
    ...
    it 'publishes via outbox when enabled' do
      ...
      result = outbox_publisher.publish(resource_type: 'user', event_type: 'created', payload: payload)
      expect(result).to be_a(JetstreamBridge::Models::PublishResult)
      expect(result.success?).to be(true)
    end
  ...
  describe 'resource_id extraction' do
    ...
    it 'extracts resource_id from payload with resource_id field' do
      ...
      result = publisher.publish(event_type: 'user.created', payload: { resource_id: 'res-789' })
      expect(result).to be_a(JetstreamBridge::Models::PublishResult)
      expect(result.success?).to be(true)
    end
  ...
  describe 'envelope normalization' do
    ...
    it 'does not override explicit resource_type' do
      ...
      result = publisher.publish(event_type: 'order.shipped', resource_type: 'custom', payload: payload)
      expect(result).to be_a(JetstreamBridge::Models::PublishResult)
      expect(result.success?).to be(true)
    end
    ...
    it 'preserves custom schema_version' do
      ...
      result = publisher.publish(event_type: 'user.created', payload: payload, schema_version: 2)
      expect(result).to be_a(JetstreamBridge::Models::PublishResult)
      expect(result.success?).to be(true)
    end
    ...
    it 'preserves custom producer' do
      ...
      result = publisher.publish(event_type: 'user.created', payload: payload, producer: 'custom-producer')
      expect(result).to be_a(JetstreamBridge::Models::PublishResult)
      expect(result.success?).to be(true)
    end
  end
